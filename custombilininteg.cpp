...
namespace mfemplus
{
    ...
    void AnisotropicElasticityDamageIntegrator::AssembleElementMatrix(
        const mfem::FiniteElement &el, mfem::ElementTransformation &Trans, mfem::DenseMatrix &elmat)
    {
        int dof = el.GetDof();
        int dim = el.GetDim();
    ...
    void AnisotropicStrainEnergyDamageIntegrator::AssembleElementMatrix(
        const mfem::FiniteElement &el, mfem::ElementTransformation &Trans, mfem::DenseMatrix &elmat)
    {
        int dof = el.GetDof();
        int dim = el.GetDim();
        ...
        for (int i = 0; i < ir->GetNPoints(); i++)
        {
            ...
            mfem::AddMult_a_VVt(w * strain_energy, shape, elmat); // multiplied by twice the strain energy - twice the pressure energy.
        }
    }
    ...
    void StVenantKirchoffTangentStiffnessIntegrator::AssembleElementMatrix(const mfem::FiniteElement &el, mfem::ElementTransformation &Tr, mfem::DenseMatrix &elmat)
    {
        ...
        for (int i = 0; i < ir->GetNPoints(); i++)
        {
            ...
            if (i == 0)
            {
                E = young_mod->Eval(Tr, ip);
                NU = poisson_ratio->Eval(Tr, ip); // The elastic constants are evaluated at each integration point.
            }
    ...
    void CompressibleNeoHookeanTangentStiffnessIntegrator::AssembleElementMatrix(const mfem::FiniteElement &el, mfem::ElementTransformation &Tr, mfem::DenseMatrix &elmat)
    {
        int dof = el.GetDof();
        int dim = el.GetDim();
        int str_comp = (dim == 2) ? 3 : 6;
        int elnum = Tr.ElementNo;
        mfem::real_t w, E, NU, LAMBDA, MU;
        ...
        for (int i = 0; i < ir->GetNPoints(); i++)
        {
            ...
            if (i == 0)
            {
                ...
                MU = mu->Eval(Tr, ip); // The elastic constants are evaluated at each integration point.
